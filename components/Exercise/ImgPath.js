//  이미지 경로들 모아놓은 파일
export const Images = {
  bench_press_machine: require("../../assets/exercise_picture/bench_press_machine.jpg"),
  chest_press_machine: require("../../assets/exercise_picture/chest_press_machine.jpg"),
  hip_abduction_machine: require("../../assets/exercise_picture/hip_abduction_machine.jpg"),
  lat_pull_down_machine: require("../../assets/exercise_picture/lat_pull_down_machine.jpg"),
  leg_extension_machine: require("../../assets/exercise_picture/leg_extension_machine.jpg"),
  leg_press_machine: require("../../assets/exercise_picture/leg_press_machine.jpg"),
  pec_deck_fly_machine: require("../../assets/exercise_picture/pec_deck_fly_machine.jpg"),
  shoulder_press_machine: require("../../assets/exercise_picture/shoulder_press_machine.jpg"),
};
